...
func toDestinationOrDefault(addr M.Socksaddr, network net.Network, fallback net.Destination) net.Destination {
	if addr.IsValid() {
		if destination, err := singbridge.ToDestination(addr, network); err == nil && destination.IsValid() {
			return destination
		}
	}
	return fallback
...
func (i *Inbound) NewConnectionEx(ctx context.Context, conn gonet.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	// Convert destination
	targetDest := toDestinationOrDefault(destination, net.Network_TCP, net.TCPDestination(net.LocalHostIP, net.Port(443)))
...
func (i *Inbound) NewPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	// Convert destination
	targetDest := toDestinationOrDefault(destination, net.Network_UDP, net.UDPDestination(net.LocalHostIP, net.Port(443)))
	...
	outConn := &singbridge.PacketConnWrapper{
		...
		Writer: link.Writer,
		Dest:   targetDest,
		T:      signal.CancelAfterInactivity(sessionCtx, cancel, 300*time.Second),
	}
